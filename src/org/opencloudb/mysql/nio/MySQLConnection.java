...
import org.opencloudb.mysql.MySQLMessage2;
import org.opencloudb.mysql.MySQLMessageWgq;
...
public class MySQLConnection extends BackendAIOConnection {
	...
	protected void sendQueryCmd1(String query,byte[] data) {
	...
	protected void sendQueryCmd2(String query,byte[] data) {
		CommandPacket packet = new CommandPacket();
		packet.packetId = 0;
		packet.command = MySQLPacket.COM_QUERY;
		byte[] added = null;
		try {			
			added = query.getBytes(charset);
		} catch (UnsupportedEncodingException e) {
			throw new RuntimeException(e);
		}
		byte[] orig = data;
		packet.arg = new byte[added.length+orig.length];
		System.arraycopy(added, 0, packet.arg, 0, added.length);
		System.arraycopy(orig, 0, packet.arg, added.length, orig.length);
		lastTime = TimeUtil.currentTimeMillis();
		packet.write(this);
	}	
	...
	protected void sendQueryCmd(String query) {
		...
		packet.command = MySQLPacket.COM_QUERY;
		try {			
			packet.arg = query.getBytes(charset);//MySQLMessageWgq.getQueryBytes(query,charset);//query.getBytes(charset);
		} catch (UnsupportedEncodingException e) {
			throw new RuntimeException(e);
		}
		lastTime = TimeUtil.currentTimeMillis();
	...
	public void execute(RouteResultsetNode rrn, ServerConnection sc,
		...
		synAndDoExecute(xaTXID, rrn, sc.getCharsetIndex(), sc.getTxIsolation(),
				autocommit,sc.getMySQLMessage2());
	}

	private void synAndDoExecute(String xaTxID, RouteResultsetNode rrn,
			int clientCharSetIndex, int clientTxIsoLation,
			boolean clientAutoCommit, MySQLMessage2 mySQLMessage2) {
		String xaCmd = null;
		...
		if (synCount == 0) {
			...
			if (mySQLMessage2 != null && mySQLMessage2.hasBinary()) {
				sendQueryCmd1(rrn.getStatement(), mySQLMessage2.getBytes());
			} else{
		...
		if (mySQLMessage2 != null && mySQLMessage2.hasBinary()) {
			...
			sendQueryCmd2(sb.toString(),mySQLMessage2.getBytes());
		}else{
			// and our query sql to multi command at last
			sb.append(rrn.getStatement());			
	...
	public void query(String query) throws UnsupportedEncodingException {
		...
		synAndDoExecute(null, rrn, this.charsetIndex, this.txIsolation, true,null);
